...
# Create the 3/4 circular obstacle
for i in range(dimx):
    for j in range(dimy):
        distance_to_center = np.sqrt((i - center[0]) ** 2 + (j - center[1]) ** 2)
        if distance_to_center < outer_rad and distance_to_center > inner_rad and not (i>dimx/2 and j<dimy/2):
            alpha[i, j] = 0  # Set obstacle
...
def main():
    ...
    for i in range(dimx):
        for j in range(dimy):
            ...
            if distance_to_center < (R_min+5) and distance_to_center > R_min and not (i>dimx/2 and j<dimy/2):
                pixeldata[i, j, :] = [255, 255, 255] #Set obstacle
    ...
    while True:
        ...
        title_text = title_font.render("2D Wave Equation Simulation - Uneven Sloshing", True, (255, 255, 255))
